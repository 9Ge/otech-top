 ...
 */
@Component
public class CCSBillingHttpLogParser implements HttpLogParser {
    ...
    public String businessNo(Object[] args) {
        try {
            if (!Arrays.isNullOrEmpty(args)) {
                Object arg = args[0];
                if (CCSBilling.class.isAssignableFrom(arg.getClass())) {
                    CCSBilling req = (CCSBilling) arg;
                    return req.getBpCodeScc();
                }
            }
        } catch (Exception e) {
        }
